...
def list_installed_packages_task(pattern=None):
    ...
    try:
        cmd = "dpkg -l"
        output = run_command(cmd, shell=True, check_output=True, ignore_errors=True)
        if not output:
            return "Error listing packages or no packages found."
...
def check_tool_installed_task(tool_name):
    ...
    try:
        ...
        path = run_command(cmd, shell=True, check_output=True, ignore_errors=True)
        if path:
